...
function snapToPx(snap: SheetSnap, sheetH: number, peekPx: number): number {
  switch (snap) {
    case 'full':
      return FULL_TOP_PX;
    case 'mid':
      return Math.max(0, sheetH - MID_VISIBLE_PX);
    case 'peek':
      return Math.max(0, sheetH - peekPx);
  }
}

function pxToNearestSnap(
  px: number,
  sheetH: number,
  peekPx: number,
  allowed: SheetSnap[] = ['full', 'mid', 'peek']
): SheetSnap {
...
function pickSnapAfterDrag(
  ...
  const order: SheetSnap[] = ['full', 'mid', 'peek'].filter((s): s is SheetSnap =>
    allowed.includes(s as SheetSnap)
  );
...
/* Should a drag-release dismiss the sheet? Only when the view configured an
   onDismiss (detail view) and the finger pulled DOWN clearly past the peek
   snap — either dragged ~64px below the peek line, or a downward flick that
...
interface SheetConfig {
  ...
  // Optional: a strong downward pull *past* the peek snap dismisses the sheet
  // (instead of parking at peek) by calling this. The detail view wires it to
  // "close detail → back to list"; the list view leaves it undefined.
  onDismiss?: () => void;
  // View is window-scrolled in-flow content on phones (≤767.98px): the hook
  // skips ALL positioning + drag work there so the window owns the gesture
  // and the sheet CSS owns the geometry. No effect on tablets/desktop.
  inflow?: boolean;
}
...
export function useBottomSheet(initial: SheetSnap = 'peek') {
  ...
  const configRef = useRef<SheetConfig>({
    maxSnap: null,
    dragMode: 'all',
    peekVisiblePx: DEFAULT_PEEK_VISIBLE_PX,
  });
  ...
  const updateControls = useCallback((root: HTMLElement, offsetPx: number) => {
    root
      .querySelectorAll<HTMLElement>('.maplibregl-ctrl-bottom-right, .maplibregl-ctrl-bottom-left')
      .forEach((c) => {
        c.style.bottom = `${offsetPx}px`;
      });
  }, []);
  ...
  */
  ...
  const applyY = useCallback(
    (px: number) => {
      const el = sheetNode.current;
      if (!el) return;
      /* Phone list view is window-scrolled in-flow — geometry belongs to the
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      if (!offset && el && isMobile() && !(configRef.current.inflow && isPhone())) {
        const h = visibleViewportH();
  ...
  const sheetRef = useCallback(
    (el: HTMLDivElement | null) => {
      ...
      if (el && isMobile() && !(configRef.current.inflow && isPhone())) {
        const h = visibleViewportH();
        const px = snapToPx(snapRef.current, h, configRef.current.peekVisiblePx);
        const visible = Math.max(0, h - px);
        el.style.setProperty('--sheet-y', `${px}px`);
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      dragRef.current = {
        ...
        v: velocityInit(e.clientY, e.timeStamp),
      };
    ...
    const onMove = (e: PointerEvent) => {
      ...
      const upperCap = maxSnap
        ? snapToPx(maxSnap, h, configRef.current.peekVisiblePx)
        : FULL_TOP_PX;
    ...
    const onUp = (e: PointerEvent) => {
      ...
      const allowed: SheetSnap[] =
        configRef.current.snaps ?? (maxSnap ? ['mid', 'peek'] : ['full', 'mid', 'peek']);
      if (dismissGesture(configRef.current, finalPx, dy, d.v.vel, h)) {
        ...
        setSnap(
          pickSnapAfterDrag(
            ...
            finalPx,
            h,
            configRef.current.peekVisiblePx,
            allowed
          )
        );
      }
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (!touchState.active) {
        ...
        if (touchState.nativeScrollOnUp && dy < 0) {
          touchState = null;
          return;
        }
        if (dy > 0 && !touchState.atScrollTop) {
          touchState = null;
          return;
        }
