...
async function main() {
  ...
  for (const r of restaurants) {
    if (!r.openingHours || r.openingHours.length === 0) {
      findings.push({
        ...
        severity: 'hoch',
        slug: r.slug,
        detail: 'keine Öffnungszeiten → kein Open-Badge, kein OpeningHours-JSON-LD',
      });
    } else if (buildOpeningHoursSpec(r.openingHours).length === 0) {
      const raw = r.openingHours.map((h) => `"${h.days} ${h.hours}"`).join(', ');
      findings.push({
        check: 'hours-unparseable',
        severity: 'hoch',
        slug: r.slug,
        detail: `Parser versteht ${raw} nicht → JSON-LD fällt still weg`,
      });
    }
    if (!r.metaTitle || !r.metaDescription) {
      findings.push({
        check: 'seo-meta-missing',
        severity: 'mittel',
        slug: r.slug,
        detail: `fehlt: ${[!r.metaTitle && 'metaTitle', !r.metaDescription && 'metaDescription'].filter(Boolean).join(', ')}`,
      });
    }
    if ((r.descLen ?? 0) < 350) {
      findings.push({
        ...
        severity: 'mittel',
        slug: r.slug,
        detail: `description nur ${r.descLen ?? 0} Zeichen`,
      });
    }
    if (!r.cuisineType) {
      findings.push({
        check: 'cuisine-type-missing',
        severity: 'mittel',
        slug: r.slug,
        detail: 'cuisineType fehlt → schwächt Suche und strukturierte Detailseiten-Fakten',
      });
    }
    if (!r.shortDescription) {
      findings.push({
        check: 'short-description-missing',
        severity: 'mittel',
        slug: r.slug,
        detail: 'shortDescription fehlt → keine kompakte Vorschau',
      });
    }
    if (!r.tip) {
      findings.push({
        ...
        severity: 'info',
        slug: r.slug,
        detail: 'Insider-Tipp fehlt im Map-Popup',
      });
    }
    if (!r.website && !r.instagramHandle) {
      findings.push({
        check: 'no-external-presence',
        severity: 'info',
        slug: r.slug,
        detail: 'weder website noch instagramHandle gepflegt',
      });
    }
    if (r.categoryCount === 0) {
      findings.push({
        check: 'categories-missing',
        severity: 'hoch',
        slug: r.slug,
        detail: 'keine Kategorie → fehlt auf allen Kategorie-Hubs und in Category-Entitlements',
      });
    }
    const hasAttributionFallback = firstPartyPhotoSlugs.has(r.slug) || !!r.instagramHandle;
    if (
      r.hasHeroPhoto &&
      !hasAttributionFallback &&
      (!r.photoCredit || !isHttpUrl(r.photoCreditUrl))
    ) {
      findings.push({
    ...
    if (r.gallery?.length) {
      ...
      if (missingAlt) {
        findings.push({
          check: 'gallery-alt-missing',
          severity: 'mittel',
          slug: r.slug,
          detail: `${missingAlt} Galerie-Bild${missingAlt === 1 ? '' : 'er'} ohne Alt-Text`,
        });
      }
  ...
  if (links) {
    const targets = restaurants.flatMap(
      (r) =>
        [
    ...
    for (let i = 0; i < targets.length; i += POOL) {
      ...
      const results = await Promise.all(
        batch.map(async (t) => ({ t, result: await checkLink(t.url) }))
      );
      for (const { t, result } of results) {
        if (result === 'dead')
          findings.push({
            ...
            severity: 'mittel',
            slug: t.slug,
            detail: `${t.kind} liefert 404/410: ${t.url}`,
          });
        if (result === 'bot-blocked')
          findings.push({
            check: 'link-bot-blocked',
            severity: 'info',
            slug: t.slug,
            detail: `${t.kind} blockt automatisierte Checks (403/429) — manuell prüfen: ${t.url}`,
          });
        if (result === 'unreachable')
          findings.push({
            check: 'link-unreachable',
            severity: 'info',
            slug: t.slug,
            detail: `${t.kind} technisch nicht prüfbar — manuell prüfen: ${t.url}`,
          });
      }
  ...
  console.log(
    `# Content-Lint — ${restaurants.length} Restaurants geprüft, ${findings.length} Befunde\n`
  );
  let lastCheck = '';
