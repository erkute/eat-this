...
describe('runBuddyTurn', () => {
  ...
  it('emits one pack teaser when results share a category, stripping categorySlugs', async () => {
        ...
        [
          {
            id: 'tu2',
            name: 'search_spots',
            input: { cuisine: 'pizza', vibe_query: 'pizza nochmal' },
          },
        ]
      ),
      turnOf(['Fertig.']),
    ...
    expect(packs[0]).toMatchObject({
      value: { packId: 'category-pizza', art: '/pics/booster/booster_pizza.webp' },
    });
        ...
        [
          {
            id: 'tu1',
            name: 'search_spots',
            input: { cuisine: 'frühstück', vibe_query: 'entspannt frühstücken' },
          },
        ]
      ),
      turnOf(['Fertig.']),
  ...
  it('emits no pack teaser when results span categories', async () => {
    ...
    const turns = [
      ...
      turnOf(
        [''],
        [{ id: 'tu1', name: 'search_spots', input: { cuisine: 'essen', vibe_query: 'essen' } }]
      ),
      turnOf(['Fertig.']),
    ];
    let i = 0;
        ...
        [
          {
            ...
            input: {
              cuisine: 'kaffee',
              bezirk: 'Mitte',
              vibe_query: 'guter kaffee in meiner nähe',
            },
          },
        ]
      ),
