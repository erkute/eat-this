...
async function triggerGuestMagicLink(email: string, locale: 'de' | 'en', stripeSessionId: string) {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (err instanceof CheckoutSessionIntegrityError) {
      ...
      return NextResponse.json({ received: true, rejected: 'session_integrity' }, { status: 200 });
    }
  ...
  if (session.payment_status !== 'paid') {
    return NextResponse.json({ received: true, pending: session.payment_status }, { status: 200 });
  }
  ...
  try {
    ...
    if (result.status === 'exists' && result.existingStripeSessionId !== session.id) {
      await refundDuplicateSession(session, result);
      return NextResponse.json({ received: true, result: 'duplicate_refunded' }, { status: 200 });
    }
    ...
    return NextResponse.json(
      { error: 'fulfill_failed', message: (err as Error).message },
      { status: 500 }
    );
  }
}
