...
describe('assembleDistricts', () => {
  it('puts favorite Berlin food districts first and marks the weekly feature in-place', () => {
    ...
    expect(out.filter((d) => d.slug !== 'neukoelln').every((d) => d.isFeature === false)).toBe(
      true
    );
  });
  ...
  it('returns favorites first with no marker when the feature slug is null or unknown', () => {
    ...
    expect(assembleDistricts('does-not-exist', rows).every((d) => d.isFeature === false)).toBe(
      true
    );
...
describe('pickWeeklyFeatureSlug', () => {
  ...
  it('is deterministic for a given date', () => {
    expect(pickWeeklyFeatureSlug(rows, '2026-06-15')).toBe(
      pickWeeklyFeatureSlug(rows, '2026-06-15')
    );
  });
