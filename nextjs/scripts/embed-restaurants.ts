...
import { EMBEDDED_RESTAURANTS_FILTER, EMBEDDINGS_PATH } from './lib/embeddings-index';
...
const QUERY = `*[${EMBEDDED_RESTAURANTS_FILTER}] | order(slug.current asc) {
  "slug": slug.current,
  name,
  cuisineType,
...
async function main() {
  ...
  const path = join(process.cwd(), EMBEDDINGS_PATH);
