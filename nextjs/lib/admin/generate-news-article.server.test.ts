...
import { generateNewsArticle, type GenerateNewsArticleInput } from './generate-news-article.server';
...
function rawArticle() {
  ...
  return {
    ...
    metaDescriptionDe:
      'Handgezogene Nudeln in Berlin: konkrete Adressen, Stile und Unterschiede in einem direkten Guide.',
    metaDescriptionEn:
      'Hand-pulled noodles in Berlin: specific addresses, styles and differences in one direct guide.',
...
describe('generateNewsArticle', () => {
  ...
  it('sends an existing Sanity hero image to the vision-capable model', async () => {
    const { client, create } = anthropicReturning(rawArticle());
    const heroImageUrl = 'https://cdn.sanity.io/images/ehwjnjr2/production/example-1200x800.jpg';
  ...
  it('supports a German-only draft', async () => {
    ...
    const result = await generateNewsArticle({ ...input, includeEnglish: false }, client);
