...
describe('computeBadges', () => {
  it('gibt einem leeren Deck nichts', () => {
    expect(computeBadges({ collected: 0, groups: [] })).toEqual([]);
  });

  /* Die erste umgedrehte Karte ist der Moment, in dem jemand tatsaechlich vor
     einem Spot stand. */
  it('feiert die erste Karte', () => {
    expect(computeBadges({ collected: 1, groups: [mitte] })).toEqual([{ kind: 'cards', value: 1 }]);
  });
  ...
  it('zeigt nur die hoechste erreichte Stufe', () => {
    expect(computeBadges({ collected: 30, groups: [] })).toEqual([{ kind: 'cards', value: 25 }]);
    expect(computeBadges({ collected: 100, groups: [] })).toEqual([{ kind: 'cards', value: 100 }]);
  });

  it('zeichnet jeden vollen Bezirk aus, die halben nicht', () => {
    expect(computeBadges({ collected: 4, groups: [kreuzberg, mitte] })).toEqual([
      { kind: 'cards', value: 1 },
  ...
  it('haelt einen leeren Bezirk nicht fuer komplett', () => {
    expect(
      computeBadges({ collected: 0, groups: [{ group: 'Spandau', done: 0, total: 0 }] })
    ).toEqual([]);
  });
  ...
  it('sagt bei einem einzigen vollen Bezirk nicht ganz Berlin', () => {
    expect(computeBadges({ collected: 3, groups: [kreuzberg] })).toEqual([
      { kind: 'cards', value: 1 },
      { kind: 'district', value: 'Kreuzberg' },
    ]);
  });
