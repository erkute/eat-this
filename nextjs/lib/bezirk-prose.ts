...
export function buildBezirkFAQEntries({ bezirk, restaurants, locale }: BezirkContext): FAQEntry[] {
  ...
  if (cats.length > 0) {
    ...
    entries.push(
      de
        ? {
            question: `Welche Restaurant-Kategorien gibt es in ${name}?`,
            answer: `In ${name}: ${list}.`,
          }
  ...
  if (restaurants.length >= 3) {
    ...
    entries.push(
      de
        ? {
            question: `Was sind bekannte Restaurants in ${name}?`,
            answer: `Aus der Auswahl: ${highlights}.`,
          }
        : {
            question: `What are some notable restaurants in ${name}?`,
  ...
  if (budget.length > 0) {
    ...
    entries.push(
      de
        ? {
            question: `Wo isst man in ${name} günstig?`,
            answer: `Im unteren Preissegment (bis 20 €): ${list}.`,
          }
        : {
            question: `Where can I eat cheap in ${name}?`,
  ...
  if (fineDining.length > 0) {
    ...
    entries.push(
      de
        ? {
            question: `Wo isst man in ${name} gehoben?`,
            answer: `Im höheren Preissegment (ab 40 €): ${list}.`,
          }
        : {
            question: `Where is fine dining in ${name}?`,
