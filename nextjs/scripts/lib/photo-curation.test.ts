...
const j = (
  index: number,
  score: number,
  category: PhotoJudgment['category'] = 'interior'
): PhotoJudgment => ({ index, score, category });
...
describe('selectGalleryPhotos', () => {
  ...
  it('never returns an all-interior ("just the shop") gallery', () => {
    expect(
      selectGalleryPhotos([j(0, 9, 'interior'), j(1, 8, 'interior')], owners([0, 1], 2), 2)
    ).toEqual([]);
  });
  ...
  it('ignores out-of-range indexes', () => {
    expect(selectGalleryPhotos([j(7, 9, 'food'), j(0, 8, 'food')], owners([0, 7], 2), 2)).toEqual([
      0,
    ]);
  });
});
...
describe('parseJudgments', () => {
  it('parses a clean JSON array', () => {
    const text =
      '[{"index":0,"category":"food","score":8.5},{"index":1,"category":"unusable","score":2}]';
