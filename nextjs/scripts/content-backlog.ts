...
function table(items: BacklogItem[], limit: number): string {
  ...
  const lines = ['| # | Restaurant | Bezirk | Score | Warum |', '|---:|---|---|---:|---|'];
...
async function main() {
  ...
  console.log(
    `- Restaurants: ${report.summary.restaurantsPublished} veröffentlicht, ${report.summary.restaurantsOpen} offen`
  );
  console.log(`- Beschreibungen: ${report.summary.descriptionsThin} unter 350 Zeichen`);
  console.log(
    `- Galerien: ${report.summary.restaurantsWithGallery} vorhanden, ${report.summary.galleryReady} direkt backfill-fähig, ${report.summary.galleryBlockedMissingPlaceId} ohne Place-ID blockiert\n`
  );
