...
async function main() {
  ...
  console.log(
    `${targets.length} restaurants need publishable map photos${dryRun ? ' (dry-run)' : ''}`
  );
  let writtenHero = 0;
  ...
  for (const [idx, target] of targets.entries()) {
    ...
    const pickedPhotos = place.photos
      .filter(
        (photo) =>
          photo.name && isOwnerPhoto(photo.authorAttributions?.[0]?.displayName, target.name)
      )
    ...
    console.log(
      `  owner photos: ${pickedPhotos.length} / ${place.photos.length}; taking first ${pickedPhotos.length}`
    );
    ...
    const patch = sanity.patch(target._id).set({
      image: {
        _type: 'image',
    ...
    console.log(
      `  written: hero + ${galleryUploads.length} gallery photo${galleryUploads.length === 1 ? '' : 's'}`
    );
  }
