      ...
      <div className={styles.inner}>
        ...
        <div className={styles.controls}>
          <ChoiceRow label={t('moodLabel')}>
            {MOODS.map((value) => (
              <Choice key={value} active={mood === value} onClick={() => setMood(value)}>
                {t(`mood.${value}`)}
              </Choice>
            ))}
          </ChoiceRow>
          <ChoiceRow label={t('budgetLabel')}>
            {BUDGETS.map((value) => (
              <Choice key={value} active={budget === value} onClick={() => setBudget(value)}>
                {t(`budget.${value}`)}
              </Choice>
            ))}
          </ChoiceRow>
          <ChoiceRow label={t('distanceLabel')}>
            {RADII.map((value) => (
              <Choice key={value} active={radius === value} onClick={() => setRadius(value)}>
                {t(`radius.${value}`)}
              </Choice>
            ))}
          </ChoiceRow>
        ...
        <div className={styles.primaryRow}>
          <button type="button" className={styles.primary} onClick={() => reveal(seed + 1)}>
            {t('go')}
          </button>
        </div>
