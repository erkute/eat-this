...
async function exportMetadata(): Promise<void> {
  ...
  const documents = await sourceClient(sourceProject, sourceDataset).fetch<
    PublicMustEatMetadata[]
  >(`
  ...
  await writeFile(output, `${sanitized.map((document) => JSON.stringify(document)).join('\n')}\n`, {
    encoding: 'utf8',
    mode: 0o600,
  });
  console.log(
...
async function scrubLegacyUnlockedDishes(db: ReturnType<typeof getFirestore>): Promise<number> {
  const snapshot = await db.collectionGroup('unlockedMustEats').get();
  const legacy = snapshot.docs.filter((document) => Object.hasOwn(document.data(), 'dish'));
...
async function verifyPrivateStore(manifest: MigrationManifest): Promise<void> {
  ...
  if (
    snapshots.length !== manifest.entries.length ||
...
async function purge(): Promise<void> {
  ...
  const client = sourceClient(manifest.publicSanityProject, manifest.publicSanityDataset, token);
...
async function verify(): Promise<void> {
  ...
  const publicClient = sourceClient(manifest.publicSanityProject, manifest.publicSanityDataset);
  ...
  if (
    publicCounts.withDish ||
    publicCounts.withDescription ||
    publicCounts.withPrice ||
    publicCounts.withImage
  ) {
