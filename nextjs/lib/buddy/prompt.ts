...
export function buildSystemPrompt(locale: Locale, opts: { hasGeo?: boolean } = {}): string {
  ...
  return [
    ...
    '5. Hat ein Spot ein `distanceLabel`, hat der Nutzer seinen Standort geteilt und die Treffer sind nach Entfernung sortiert (nächster zuerst). Sei EHRLICH mit der Entfernung — nenne sie konkret und verkaufe nichts Weites als „in der Nähe". Nur wirklich nahe Spots (unter ~1 km) sind „um die Ecke"/„nur 400 m von dir". Ist auch der NÄCHSTE weiter weg (über ~1,5 km), sag das offen, statt Nähe vorzutäuschen — z.B. „Ganz in der Nähe ist gerade nichts dabei — das Nächste ist 2,1 km weg, aber das lohnt den Weg:". Erfinde nie eine kürzere Entfernung als im `distanceLabel`.',
    '',
    '## Ton & Stil (WICHTIG)',
