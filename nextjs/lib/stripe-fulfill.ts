...
export async function assembleAndWriteEntitlement({
  ...
  packId,
  stripeSessionId,
}: Args): Promise<FulfillmentResult> {
  const pack = getPack(packId);
  if (!pack) throw new Error(`unknown pack: ${packId}`);

  const db = getAdminFirestore();
  const ref = db.collection('users').doc(uid).collection('entitlements').doc(packId);
    ...
    if (snap.exists) {
      ...
      return {
        ...
        existingStripeSessionId:
          typeof existing?.stripeSessionId === 'string' ? existing.stripeSessionId : null,
